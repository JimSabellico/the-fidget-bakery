...
export default async function handler(req, res) {
  ...
  const kind = ['message', 'custom', 'teacher'].includes(requestedKind) ? requestedKind : 'message';
  ...
  const label = kind === 'custom' ? 'Custom order' : kind === 'teacher' ? 'Teacher discount request' : 'Message';
