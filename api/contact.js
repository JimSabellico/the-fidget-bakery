...
export default async function handler(req, res) {
  ...
  const body = `${kind === 'custom' ? 'CUSTOM ORDER REQUEST' : 'NEW MESSAGE'}\n\nName: ${name}\nEmail: ${email}\nDetails: ${details || 'None provided'}\n\nMessage:\n${message}`;
  try {
    const response = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { Authorization: `Bearer ${process.env.RESEND_API_KEY}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ from: process.env.CONTACT_FROM_EMAIL, to: [process.env.CONTACT_TO_EMAIL], reply_to: email, subject: `${kind === 'custom' ? 'Custom order' : 'Message'} from ${name} — The Fidget Bakery`, text: body })
    });
