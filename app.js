...
try {
  ...
  if (Array.isArray(stored)) {
    ...
    cart = stored.filter(entry => {
      ...
      if (entry && legacyFidgetIds[entry.id]) entry.id = legacyFidgetIds[entry.id];
...
const image = (item, className = '') => `<img class="${className} ${item.imageKind === 'photo' ? 'real-product-photo' : item.imageKind === 'pending' ? 'pending-product-photo' : 'product-visualization'}" src="${item.image}" alt="${item.imageKind === 'photo' ? 'Photo of' : item.imageKind === 'pending' ? 'Product photo coming soon for' : 'Illustration of'} ${item.name}" loading="lazy">`;
...
function home() {
  const cookie = catalog.find(item => item.id === 'chocolate-chip-cookie'), dumpling = catalog.find(item => item.id === 'dumpling');
  ...
  <section class="home-choices page-wrap"><div class="section-title"><span class="eyebrow">PICK YOUR HAPPY</span><h2>What sounds <em>fun?</em></h2></div><div class="choice-grid">
    <a class="choice choice-fidgets" href="/fidgets"><div class="choice-copy"><span class="choice-number">01 / SPIN + SLIDE</span><h3>Fidgets</h3><p>Cookie spinners, sliders, and little moments of joy.</p><span class="choice-action">Explore fidgets <b>↗</b></span></div>${image(catalog.find(item => item.id === 'rainbow-cookie'))}${sparkle('choice-spark')}</a>
...
function collection(category) {
  ...
  const allItems = catalog.filter(item => item.category === category);
  const requestedType = new URLSearchParams(location.search).get('type');
  const selectedType = category === 'fidgets' && ['slider', 'spinner'].includes(requestedType) ? requestedType : 'all';
  const items = selectedType === 'all' ? allItems : allItems.filter(item => item.fidgetType === selectedType);
  const filters = category === 'fidgets' ? `<nav class="catalog-filters" aria-label="Filter fidgets by type"><a href="/fidgets" class="${selectedType === 'all' ? 'active' : ''}" ${selectedType === 'all' ? 'aria-current="page"' : ''}>All fidgets <span>${allItems.length}</span></a><a href="/fidgets?type=slider" class="${selectedType === 'slider' ? 'active' : ''}" ${selectedType === 'slider' ? 'aria-current="page"' : ''}>Magnet sliders <span>${allItems.filter(item => item.fidgetType === 'slider').length}</span></a><a href="/fidgets?type=spinner" class="${selectedType === 'spinner' ? 'active' : ''}" ${selectedType === 'spinner' ? 'aria-current="page"' : ''}>Spinners <span>${allItems.filter(item => item.fidgetType === 'spinner').length}</span></a></nav>` : '';
  return `<section class="collection-hero ${category}"><div class="page-wrap collection-hero-inner"><div><div class="breadcrumbs">${crumb('Home', '/')}<span>${data.name}</span></div><span class="eyebrow">${data.eyebrow}</span><h1>${data.title}</h1><p>${data.intro}</p><span class="collection-price">${data.price}</span></div><div class="collection-hero-art">${image(items[0])}<span class="collection-bubble">${category === 'fidgets' ? selectedType === 'slider' ? 'SLIDE!' : selectedType === 'spinner' ? 'SPIN!' : 'PLAY!' : 'HI!'}</span>${sparkle('collection-spark')}</div></div></section>
  <section class="collection-list page-wrap"><div class="list-heading"><div><span class="eyebrow">FRESH FROM THE BAKERY</span><h2>Meet every <em>${data.name.slice(0,-1)}.</em></h2></div><span>${items.length} little ${category === 'fidgets' ? 'joys' : 'friends'} to explore</span></div>${filters}<div class="products-grid">${items.map(productCard).join('')}</div></section>
...
function product(item) {
  ...
  return `<section class="product-page page-wrap"><div class="breadcrumbs">${crumb('Home','/')}${crumb(category.name,`/${item.category}`)}<span>${item.name}</span></div><div class="product-layout">
    ...
    ${item.makerworld ? `<a class="quiet-link" href="${item.makerworld}" target="_blank" rel="noopener noreferrer">See the original design on MakerWorld ↗</a>` : ''}<p class="image-note">${item.imageKind === 'photo' ? 'Photographed product. Color and finish may vary slightly.' : item.imageKind === 'pending' ? 'Product photo coming soon.' : 'Wiggle image is an illustration; final 3D prints may vary.'}</p></div>
...
function cartPage() {
  ...
  const rows = cart.map(entry => {
    ...
    return `<div class="cart-item"><a href="${productUrl(item)}">${image(item)}</a><div><span class="mini-label">${item.category === 'fidgets' ? item.type : 'Wiggle'}</span><h3><a href="${productUrl(item)}">${item.name}</a></h3><span>${money(item.priceCents)} each</span><div class="quantity"><button type="button" data-change="${item.id}" data-delta="-1" aria-label="Remove one ${item.name}">−</button><span>${entry.quantity}</span><button type="button" data-change="${item.id}" data-delta="1" aria-label="Add one ${item.name}">+</button></div></div><strong>${money(item.priceCents * entry.quantity)}</strong></div>`;
  }).join('');
